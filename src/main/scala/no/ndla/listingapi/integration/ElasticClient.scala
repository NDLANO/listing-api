...
object JestClientFactory {
  def getClient(searchServer: String = ListingApiProperties.SearchServer): NdlaJestClient = {
    ListingApiProperties.RunWithSignedSearchRequests match {
  ...
  private def getSigningClient(searchServer: String): NdlaJestClient = {
    ...
    val awsSigner = new AWSSigner(new DefaultAWSCredentialsProviderChain(), ListingApiProperties.SearchRegion, "es", clock)
