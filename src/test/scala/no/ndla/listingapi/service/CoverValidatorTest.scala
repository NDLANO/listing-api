...
class CoverValidatorTest extends UnitSuite with TestEnvironment {
  ...
  test("validate returns a failure if title contains html or an unknown language") {
    ...
    service
      .validate(sampleCover.copy(title = Seq(Title("Ash nazg durbatulûk", "black speech"))))
      .isFailure should be(true)
  }

  test("validate returns a failure if description contains html or an unknown language") {
    service
      .validate(sampleCover.copy(description = Seq(Description("<h1>title</h1>", "nb"))))
      .isFailure should be(true)
    service
      .validate(sampleCover.copy(description = Seq(Description("Ash nazg durbatulûk", "black speech"))))
      .isFailure should be(true)
  }
  ...
  test("validate returns a failure if a coverPhoto is a link to a non-ndla domain") {
    service
      .validate(sampleCover.copy(coverPhotoUrl = "http://nonndla.org/198273918237.jpg"))
      .isFailure should be(true)
  }
