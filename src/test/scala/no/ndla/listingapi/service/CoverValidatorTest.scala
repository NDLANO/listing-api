...
class CoverValidatorTest extends UnitSuite with TestEnvironment {
  ...
  test("validate returns a failure if title contains html or an unknown language") {
    service.validate(sampleCover.copy(title=Seq(Title("<h1>title</h1>", "nb")))).isFailure should be(true)
    service.validate(sampleCover.copy(title=Seq(Title("Ash nazg durbatulûk", "black speech")))).isFailure should be(true)
  }

  test("validate returns a failure if description contains html or an unknown language") {
    service.validate(sampleCover.copy(description=Seq(Description("<h1>title</h1>", "nb")))).isFailure should be(true)
    service.validate(sampleCover.copy(description=Seq(Description("Ash nazg durbatulûk", "black speech")))).isFailure should be(true)
  }

  test("validate returns a failure if a label contains html or an unknown language") {
    val labelsWithHtml = LanguageLabels(Seq(Label(None, Seq("<h1>label</h1>"))), "nb")
    val labelsWithHtml2 = LanguageLabels(Seq(Label(Some("<h1>category</h1>"), Seq("label"))), "nb")
    val labelsWithInvalidLang = LanguageLabels(Seq(Label(None, Seq("ash", "nazg"))), "black speech")
